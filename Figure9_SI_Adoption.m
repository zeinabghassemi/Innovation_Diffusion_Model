%% Supplementary Figure S9: Zoomed Adoption Dynamics
%
% This script reproduces the zoomed-in adoption trajectories
% presented in Supplementary Figure S9.
%
% The figure compares long-term convergence of adoption dynamics
% under different levels of intergroup interaction.
%
% MATLAB version:
% MATLAB R2024b
...
ylabel('Overall Adoption', 'FontName', 'Times New Roman', 'FontSize', 12);
...
inset_ax = axes('Position', [0.58 0.23 0.28 0.25]);
box on;
hold on;
grid on;

plot(x_zoom, Z1(x_zoom), 'r', 'LineWidth', 2);
plot(x_zoom, Z2(x_zoom), '--b', 'LineWidth', 2);
plot(x_zoom, Z3(x_zoom), ':g', 'LineWidth', 2);
plot(x_zoom, Z4(x_zoom), '-.k', 'LineWidth', 2);
...
xlabel('Time', 'FontName', 'Times New Roman', 'FontSize', 9);
ylabel('Overall Adoption', 'FontName', 'Times New Roman', 'FontSize', 9);

xticks([80 100 120]);
ytick_values = linspace(y_min, y_max, 3);
yticks(ytick_values);
ytickformat('%.3f');
title('Zoomed-in', 'FontName', 'Times New Roman','FontSize', 9);
set(gca, 'FontSize', 8);
...
% Data from the table
a = [0.5, 0.45, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.05, 0]; % x-axis values (parameter h)
Dispchcl = [34.21, 35.05, 35.94, 36.85, 37.82, 38.86, 40, 41.3, 42.89, 45.08, 49.42]; % y-axis values
